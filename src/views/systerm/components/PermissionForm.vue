...
export default {
  ...
  methods: {
    getRoleDetail() {
      return this.$api.role.getRoleDetail({ id: this.detail.id })
        .then(res => {
          if (!res) return false
    ...
    formatCheckOptions(menus) {
      ...
      for (let i = 0; i < this.checkOptions.length; i++){
        ...
        this.checkOptions[i]['checkAll'] = checkedList.length === options.length&&(options.length>0||this.detailChecked.toString().includes(par.value))
      }
    },
    // 初始化表单
    initForm() {
    ...
    submitForm() {
      const { menuPermissionCodes, menuElementPermissionCodes } = this.getPermission()
