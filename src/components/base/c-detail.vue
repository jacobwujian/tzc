...
<template>
  <div>
    ...
    >
      <a-descriptions
        :title="lItem.title"
        :column="lItem.children ? 1 : (column || 3)"
        :layout="type"
      >
        <template v-if="lItem.children">
          <a-descriptions-item
            v-for="(sItem, index) in lItem.children"
            :key="index"
          >
            <a-descriptions
              :title="sItem.title"
            ...
            >
              <a-descriptions-item
                v-for="(ssItem, ssIndex) in sItem.list"
                :key="ssIndex + lIndex"
                :label="ssItem.label"
              >
                <Star
                  v-if="ssItem.type==='rate'"
                  :count="ssItem.value"
                />
        ...
        <template v-else>
          ...
          >
            ...
            <template v-else-if="sItem.type==='approveRemark'">
              ...
              >
                <a-textarea
                  v-model.trim="sItem.value"
                  :disabled="sItem.formType!=='review'"
            ...
            <template v-else-if="sItem.type==='radio'">
              ...
              >
                ...
                >
                  ...
                  >
                    <a-radio
                      v-for="(op, i) in sItem.dataSource"
                      :key="`${i}-${op.value}`"
                      :value="op.value"
                    >
                      {{ op.name }}
            ...
            <template
              v-else-if="sItem.type === 'fileInfos'"
            >
              <a-form-model-item
                label="协议与付款凭证:"
              >
